...
const processSnapshotForLinkPreview = async ({
  admin,
  db,
  logger,
  event,
  ...
  try {
    ...
    const rawErrorName =
      typeof error?.name === "string" && error.name.trim() ? error.name : null;
    await writeMessageFailed({
      admin,
      messageRef: snapshot.ref,
      normalizedUrl,
...
const buildLinkPreviewHandler =
  ...
  async (event) => {
    ...
    if (!snapshot) return;
    await processSnapshotForLinkPreview({
      admin,
      db,
      logger,
      event,
...
const buildLinkPreviewUpdateHandler =
  ({ admin, db, logger }) =>
  async (event) => {
    ...
    const previousMessage = before?.data?.() || {};
    const nextMessage = after.data() || {};

    // Avoid loops for function-written preview updates; only react to text/type changes.
    if (
      previousMessage.type === nextMessage.type &&
      previousMessage.msg === nextMessage.msg
    ) {
      return;
    }
